...
<div class="AlonModule">
 <div class="AlonModule_Top">
          	 ...
          	 <div class="AlonModule_Top2">
                 <ul>
                         ...
                         <li class="currentHover"><a>>>修改升级任务</a></li>
                 </ul>
          	 </div>
 ...
 <div class="AlonModule_Body">
   ...
   <s:form action="update" namespace="/softup"  method="post" enctype="multipart/form-data"   onsubmit="return checkForm();">
   <table width="98%" class="listtable">
  <tr class="TableTitle">
    <th scope="row"  width="10%" >名称：</th>
    <td class="inp">
     <s:hidden name="softUpdateTask.id"/>
    <input type="text" name="softUpdateTask.name" id="name" maxlength="30" value="${softUpdateTask.name}"/>
  ...
  <tr>
    <th scope="row">状态：</th>
    <td>
    <s:if test="%{softUpdateTask.status==1}">
    <input type="radio" name="softUpdateTask.status" value="1" checked="checked">启用
    </s:if>
    <s:else>
   	  <input type="radio" name="softUpdateTask.status" value="1"/>启用
    </s:else>
    <s:if test="%{softUpdateTask.status==2}">
      <input type="radio" name="softUpdateTask.status" value="2" checked="checked"/>停止
    </s:if>
    <s:else>
   ...
   <tr>
    ...
    <td>
    	从 
    	<input id="startTime" name="softUpdateTask.startTime"  value="<s:date name="softUpdateTask.startTime" format="yyyy-MM-dd HH:mm:ss"/>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd H:m:s'})"/>
    	到
    	<input id="endTime" name="softUpdateTask.endTime" value="<s:date name="softUpdateTask.endTime" format="yyyy-MM-dd HH:mm:ss"/>"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd H:m:s'})"/>
    	<font style="color: red; margin-left: 10">*</font>
    </td>
  </tr>
  ...
  <tr>
    <th scope="row">描述：</th>
    <td> 
    	<s:textarea id="remark" rows="5" cols="50" name="softUpdateTask.remark"></s:textarea>
    </td>
  </tr>
  <tr>
		 ...
		 <s:iterator value="categories" id="item">
		   ...
		   <ul style="background-color:#EDEBEB;">
		  	    <s:iterator value="devices" id="items">
		  	 	  <li>
		   		   <input class="check${categoryId}" type="checkbox"  <s:if test="deviceMap[id] != null">checked=true</s:if>  name="idListsave"  value="${items.id}"/>
  ...
  <td>  <input  class="submit"  type="submit" value="提交"/> <input type="button"   class="back" value="返回" onclick="window.location.href='<%=basePath%>softup/list.action'"/></td>
  </tr>
</table>
</s:form>
</div>
...
<div class="AlonModule">
 ...
 <div class="AlonModule_Body">
  ...
  <tr>
    ...
    <td>
    <s:if test="%{softUpdateTask.status==1}">
    <input type="radio" name="softUpdateTask.status" value="1" checked="checked">Start 
    </s:if>
    <s:else>
   	  <input type="radio" name="softUpdateTask.status" value="1"/>Start 
    </s:else>
    <s:if test="%{softUpdateTask.status==2}">
      <input type="radio" name="softUpdateTask.status" value="2" checked="checked"/>stop
    </s:if>
    <s:else>
   ...
   <tr>
    <th scope="row">Time to upgrade:</th>
    <td>
    	from
    	<input id="startTime" name="softUpdateTask.startTime"  value="<s:date name="softUpdateTask.startTime" format="yyyy-MM-dd HH:mm:ss"/>" onclick="WdatePicker({dateFmt:'yyyy-MM-dd H:m:s'})"/>
    	to
  ...
  <td>  <input  class="submit"  type="submit" value="submit"/> <input type="button"   class="back" value="return" onclick="window.location.href='<%=basePath%>softup/list.action'"/></td>
  </tr>
</table>
</s:form>
</div>
