...
public class MyFileUploadInterceptor extends AbstractInterceptor {
    ...
    private static final String UPLOAD_TAG_ID = "boyue_fileupload_";
    ...
    public String intercept(ActionInvocation invocation) throws Exception {
    	//文件上传map
    	Map<String, List<File>> UploadFileMap = new HashMap<String, List<File>>(); 
                ...
                if (isNonEmpty(fileName)) {
                    ...
                    if (files != null && files.length > 0) {
                        ...
                        if (!acceptedFiles.isEmpty()) {
                            ...
                            if (inputName.startsWith(UPLOAD_TAG_ID)) {
                            	//要设置最后保存路径的字段
	                            String targetField = inputName.replace(UPLOAD_TAG_ID, "");
	                            //文件保存路径
	                            String savePath = request.getParameter(UPLOAD_TAG_ID + "savepath_" + targetField);
	                            //保存之后，多个用逗号分开
	                            List<String> filePaths = saveMyFile(invocation, savePath,acceptedFiles, acceptedFileNames);
	                            params.put(targetField, StringUtils.join(filePaths, ","));
                            }
                            //boyue_fileupload_file_
                        }
                    }
                } else {
    ...
    private List<String> saveMyFile(ActionInvocation invocation,String savePath, List<File> files,List<String> fileNames) throws IOException {
    	String basepath = ServletActionContext.getServletContext().getRealPath("") + "/";
    	List<String> filePaths = new ArrayList<String>();
    	for (int i = 0; i < files.size(); i++) {
    		if (files != null && !files.isEmpty()) {
    			String filename = savePath + System.currentTimeMillis() 
    					+ "." +FilenameUtils.getExtension(fileNames.get(i)).toLowerCase();
    			FileUtils.copyFile(files.get(i), new File(basepath + filename));
